...
(function(window, document, $, undefined) {
  ...
  $.extend(F, {
    ...
    bindEvents() {
      ...
      W.bind(
        `orientationchange.fb${isTouch ? '' : ' resize.fb'}${
    ...
    trigger(event, o) {
      ...
      if (obj) {
        if ($.isFunction(obj[event])) {
          ret = obj[event](...Array.prototype.slice.call(arguments, 1));
        }
    ...
    _start(index) {
      ...
      coming.wrap = $(coming.tpl.wrap)
        .addClass(
          ...
          } fancybox-type-${type} fancybox-tmp ${coming.wrapCSS}`
        )
  ...
  D.ready(() => {
    ...
    if ($.support.fixedPosition === undefined) {
      $.support.fixedPosition = (function() {
        let elem = $('<div style="position:fixed;top:20px;"></div>').appendTo(
            'body'
          ),
